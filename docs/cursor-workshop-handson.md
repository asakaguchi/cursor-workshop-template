...
| 1:15-1:25 | クラウド公開 | 作ったアプリをインターネット経由で使えるようにする | AI がクラウド設定を自動でやってくれる |
...
1. **アプリをパッケージ化**（Docker化：アプリケーションをコンテナとして実行可能な形式にパッケージング）
2. **クラウドにアップロード**（Google Cloud Platform へ送信）
