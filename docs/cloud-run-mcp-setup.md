# Google Cloud Run MCP セットアップガイド

このドキュメントでは、Google Cloud Run MCPをClaude CodeとCursorエディタで使用するための設定方法を説明します。セキュリティ上の理由から、サービスアカウントキーを使用せず、よりセキュアな認証方式を採用しています。
...
- **専用権限**: Cloud Run用の専用サービスアカウントを使用
- **短期間有効**: 一時的な認証情報を使用
...
gcloud auth application-default login \
  --impersonate-service-account=cloud-run-mcp@YOUR_PROJECT_ID.iam.gserviceaccount.com
```
...
この場合、devcontainer内では追加設定不要でCloud Run MCPが利用可能になります。
...
この場合、devcontainerを起動するだけでCloud Run MCPが利用可能になります。
...
4. 「Deploy to Cloud Run」などのコマンドが使用可能になります
...
4. AIアシスタントを通じてCloud Run関連のコマンドを実行
...
# インパーソネーション権限の確認
gcloud iam service-accounts get-iam-policy cloud-run-mcp@$PROJECT_ID.iam.gserviceaccount.com
```
...
gcloud auth application-default login \
  --impersonate-service-account=cloud-run-mcp@$PROJECT_ID.iam.gserviceaccount.com
```
...
- **専用サービスアカウント**: Cloud Run専用の権限を持つサービスアカウントを使用
